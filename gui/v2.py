...
def get_user_inputs_dynamic(fields):
    ...
    for i, field in enumerate(fields):
        ...
        tk.Label(root, text=label_text, fg=fg_color)\
            .grid(row=i, column=0, padx=5, pady=5, sticky="nw")

        if ftype == "dropdown":
            combo = ttk.Combobox(root, values=field.get("options", []), width=67, state="readonly")
            combo.grid(row=i, column=1, padx=5, pady=5, sticky="nw")
            ...
            for option in options:
                tk.Radiobutton(radio_frame, text=option, variable=var, value=option)\
    ...
    tk.Button(root, text="Submit", command=submit)\
        .grid(row=len(fields) + 1, column=0, columnspan=4, pady=20)

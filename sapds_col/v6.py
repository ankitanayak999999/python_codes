...
#Added SOURCE_OBJECT for DB rows (datastore.owner.table) placed before MAPPING_TEXT
...
# ---------- view-name helper for ST ----------
def _io_view_name(node, pm):
    """Return nearest DIOutputView/DIInputView name; else ''."""
    # search children first
    for ch in node.iter():
        t = lower(strip_ns(getattr(ch, "tag", "")))
        if t in ("dioutputview","diinputview"):
            nm = (attrs_ci(ch).get("name") or "").strip()
            if nm: return nm
    # then look around ancestors' children
    for anc in ancestors(node, pm, 100):
        for ch in anc:
            t = lower(strip_ns(getattr(ch, "tag", "")))
            if t in ("dioutputview","diinputview"):
                nm = (attrs_ci(ch).get("name") or "").strip()
...
def collect_source_target_refs(root):
    ...
    Returns: list of dicts (df, schema, role, kind, dsC/ownerC/tableC for DB + view/raw names).
    """
    ...
    for n in root.iter():
        ...
        # raw parts for DB & view name for nicer TRANSFORMATION_NAME
        raw_ds   = a.get("datastorename") or a.get("datastore") or ""
        raw_owner= a.get("ownername")     or a.get("owner") or a.get("schema") or ""
        raw_tbl  = a.get("tablename")     or a.get("table") or a.get("name") or ""
        view_nm  = _io_view_name(n, pm) or schema_name
        ...
        refs.append({
            ...
            "kind": kind,
            "dsC": _canon(raw_ds),
            "ownerC": _canon(raw_owner),
            "tableC": _canon(raw_tbl),
            "dsRaw": raw_ds,
            "ownRaw": raw_owner,
            "tabRaw": raw_tbl,
        })
...
def _extract_source_columns(mapping_text: str) -> str:
    ...
    # capture table.column with optional quoting/brackets
    pattern = r"""
        (?:
    ...
    """
    matches = re.findall(pattern, txt, flags=re.VERBOSE)
    seen, ordered = set(), []
    for m in matches:
    ...
    for e in root.iter():
        ...
        if tag == "dielement" and in_dataflow(e, pm):
            ...
            row = {
                ...
                "SOURCE_OBJECT": "",                      # not applicable for TRANSFORM
...
def collect_db_st_rows(root, df_to_job, job_to_project, df_to_project, rows):
    ...
    for r in st_refs:
        ...
        # fully-qualified object (if all parts present)
        src_obj = (f"{r['dsRaw']}.{r['ownRaw']}.{r['tabRaw']}"
                   if r["dsRaw"] and r["ownRaw"] and r["tabRaw"] else "")
        ...
        for col in sorted(cols):
            ...
            row = {
                ...
                "JOB_NAME": job,
                "DATAFLOW_NAME": r["df"],
                "TRANSFORMATION_NAME": r.get("view_name") or r["schema"],  # prefer IO view name
...
def collect_file_excel_st_rows(root, df_to_job, job_to_project, df_to_project, rows):
    ...
    for r in st_refs:
        ...
        row = {
            ...
            "JOB_NAME": job,
            "DATAFLOW_NAME": r["df"],
            "TRANSFORMATION_NAME": r.get("view_name") or r["schema"],  # prefer IO view
            "TRANSFORMATION_TYPE": r["role"],           # FILE/EXCEL append
            "TARGET_COLUMN": "",
            "SOURCE_COLUMNS": "",
            "SOURCE_OBJECT": "",                        # no DB object for file/excel
            "MAPPING_TEXT": "",
        }
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    if rows:
        df = pd.DataFrame(rows, columns=[
            ...
            "TARGET_COLUMN","SOURCE_COLUMNS","SOURCE_OBJECT","MAPPING_TEXT"  # SOURCE_OBJECT before MAPPING_TEXT
        ])
    ...
    empty_df=pd.DataFrame(columns=[
        "PROJECT_NAME","JOB_NAME","DATAFLOW_NAME",
        "TRANSFORMATION_NAME","TRANSFORMATION_TYPE",
        "TARGET_COLUMN","SOURCE_COLUMNS","SOURCE_OBJECT","MAPPING_TEXT"])
    return empty_df

# ======================================================================
# 5) main()  — SAME LAYOUT AS YOUR V5 (only added new col + rename here)
# ======================================================================
def main():
    ...
    # RENAME (added SOURCE_OBJECT, order keeps SOURCE_OBJECT before MAPPING_TEXT)
    rename_mapping = {
        ...
        'SOURCE_OBJECT'      : 'SOURCE_OBJECT',   # <--- NEW

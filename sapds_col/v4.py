# sapds_column_mapping_v3_strict3key.py
# Column-level extractor for SAP DS XML exports
# - TRANSFORMATION_TYPE: SOURCE / TRANSFORM / TARGET
# - TRANSFORMATION_LEVEL: schema order within each dataflow
# - Columns for SOURCE/TARGET only when (datastore, owner, table) matches a global <DITable>
# - No fallbacks (by design): avoids wrong mappings when table names repeat
...
# Source/Target container tags used inside DISchema
SOURCE_CONTAINER_TAGS = (
    "didatabasetablesource", "difilesource", "diexcelsource",
    "ditablesource"  # some exports use this
)
...
# ---------------- context helpers (aligned with your table script) ----------------
...
def build_table_catalog(root):
    """
    Returns by3[(DS, OWNER, TABLE)] -> set(columns)
    Keys are uppercased, trimmed, and de-spaced.
    """
    by3 = defaultdict(set)
    for n in root.iter():
        ...
        tbl   = a.get("name") or a.get("table_name") or ""
        owner = a.get("owner") or a.get("ownername") or ""
        # SAP DS may also include database/db_alias_name; we stick to datastore for strict match
        ds    = a.get("datastore") or a.get("datastorename") or ""

        tblC, ownerC, dsC = canon(tbl), canon(owner), canon(ds)
        if not (tblC and ownerC and dsC):
            # require all three for strict catalog
            continue
...
def extract_db_ref_from_schema(schema_node):
    """
    Return (role, ds, owner, table) if a DB/file/Excel source/target node exists beneath schema.
    We only use ds/owner/table, and will do strict 3-key join.
    """
...
# ---------------- DF structures: schema order + explicit DIElements ----------------
def compute_df_structures(root):
    ...
    for df_node in root.iter():
        ...
        for sc in df_node.iter():
            ...
            # find a strict Source/Target container & pull its db reference
...
def transformation_type_by_tags(e, pm):
    # For DIElements: decide if they live under a source/target container; else TRANSFORM
    for a in ancestors(e, pm, 200):
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    # Structures
    table_by3 = build_table_catalog(root)                 # STRICT catalog
    ...
    # Pass 1: explicit DIElement rows (TRANSFORM or S/T if DIElements exist under them)
    for e in root.iter():
    ...
    # Pass 2: add SOURCE/TARGET schema columns using STRICT 3-key catalog only
    for df_name, schemas in df_levels.items():
        ...
        for sch_name, lvl in schemas.items():
            ...
            key = (canon(ds), canon(owner), canon(tbl))
            cols = table_by3.get(key, set())  # STRICT: only 3-key

            if not cols:
                # strict mode: skip if not found
                continue

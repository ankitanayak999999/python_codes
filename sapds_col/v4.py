...
# Column-level extractor using same context logic as your table script
# + TRANSFORMATION_TYPE (SOURCE/TARGET/TRANSFORM)
...
# --- SAME tags you used in table-level script for source/target detection ---
...
# ---- NEW: same tag-based logic as your table script ----
def transformation_type_for(e, pm):
    for a in ancestors(e, pm, 200):
        ...
        if t == "dischema":  # stop at transform boundary
            break
    return "TRANSFORM"

# ---- NEW: simple per-DF schema order levels (no link parsing) ----
def compute_df_schema_levels(root):
    """
    Returns {dataflow_name: {schema_name: level}} where level is 1..N
    based on first-appearance order of DISchema inside each Dataflow.
    """
    df_schema_levels = {}
        ...
        if lower(strip_ns(getattr(df_node, "tag", ""))) in DF_TAGS:
            ...
            for n in df_node.iter():
                if lower(strip_ns(getattr(n, "tag", ""))) == "dischema":
                    na = attrs_ci(n)
                    nm = (na.get("name") or na.get("displayname") or "").strip()
                    if nm and nm not in level_map:
                        level_map[nm] = next_level
                        next_level += 1
            if level_map:
                df_schema_levels[df_name] = level_map
    return df_schema_levels
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    # NEW: pre-compute simple schema-order levels per DF
    df_schema_levels = compute_df_schema_levels(root)
    ...
    for e in root.iter():
        ...
        if tag == "dielement" and in_dataflow(e, pm):
            ...
            # level from per-DF schema order (fallback to 1 if not found)
            lvl = 1

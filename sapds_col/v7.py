# 08/28 - ALL TRANSFORMATION WORKING FILE, SOURCE COLUMNS ADDED, SOURCE AND TARGET ROWS ADDED, FILES ROWS ADDED
# Updates (v6):
# - TRANSFORMATION_NAME: TRANSFORM -> schema; SOURCE -> DIOutputView; TARGET -> 'Target - Transformation'
# - OBJECT_NAME: for SOURCE/TARGET (DB) -> DSNAME.TABLENAME.COLUMNNAME ; TRANSFORM/FILE/EXCEL -> ""
# - SOURCE_COLUMN_COUNT added right after SOURCE_COLUMNS
...
def collect_source_target_refs(root):
    ...
    Returns: list of dicts (df, schema, outview, role, kind, dsC/ownerC/tableC for DB).
    """
    ...
    for n in root.iter():
        ...
        # backtrack to find DF & DISchema and DIOutputView
        df_name=schema_name=outview_name=""
        schema_node=None
        ...
        if schema_node is not None:
            for ch in schema_node.iter():
                if lower(strip_ns(getattr(ch,"tag",""))) == "dioutputview":
                    candidate=(attrs_ci(ch).get("name") or "").strip()
                    if candidate:
                        outview_name=candidate
                        break
        ...
        refs.append({
            "df": df_name,
            "schema": schema_name,
            "outview": outview_name,   # <--- captured DIOutputView name
            "role": role,
            "kind": kind,
    ...
    for e in root.iter():
        ...
        if tag == "dielement" and in_dataflow(e, pm):
            ...
            row = {
                ...
                "JOB_NAME": job,
                "DATAFLOW_NAME": df,
                "TRANSFORMATION_NAME": transform,   # schema name
...
def collect_db_st_rows(root, df_to_job, job_to_project, df_to_project, rows):
    ...
    for r in st_refs:
        ...
        trans_name = r["outview"] if r["role"] == "SOURCE" and r["outview"] else "Target - Transformation"

        key = (r["dsC"], r["ownerC"], r["tableC"])
        cols = table_catalog.get(key, set())
        for col in sorted(cols):
            obj_name = f"{r['dsC']}.{r['tableC']}.{col}"  # DS.TABLE.COLUMN (canonical)
            ...
            row = {
                ...
                "TRANSFORMATION_TYPE": r["role"],        # SOURCE / TARGET
                "TARGET_COLUMN": col,
                "SOURCE_COLUMNS": "",
...
def collect_file_excel_st_rows(root, df_to_job, job_to_project, df_to_project, rows):
    ...
    for r in st_refs:
        ...
        row = {
            ...
            "TARGET_COLUMN": "",
            "SOURCE_COLUMNS": "",
            "OBJECT_NAME": "",     # no column-level object for file/excel
            "MAPPING_TEXT": "",
        }
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    if rows:
        df = pd.DataFrame(rows, columns=[
            ...
            "TARGET_COLUMN","SOURCE_COLUMNS","OBJECT_NAME","MAPPING_TEXT"
        ])

        # --- add SOURCE_COLUMN_COUNT right after SOURCE_COLUMNS ---
        df["SOURCE_COLUMN_COUNT"] = df["SOURCE_COLUMNS"].apply(
    ...
    empty_df = pd.DataFrame(columns=[
        "PROJECT_NAME","JOB_NAME","DATAFLOW_NAME",
        "TRANSFORMATION_NAME","TRANSFORMATION_TYPE",
        "TARGET_COLUMN","SOURCE_COLUMNS","OBJECT_NAME","MAPPING_TEXT","SOURCE_COLUMN_COUNT"
    ])
    return empty_df

# ======================================================================
# 5) main() — SAME LAYOUT (only added new cols to rename/order)
# ======================================================================
def main():
    ...
    # RENAME / ORDER (added OBJECT_NAME, SOURCE_COLUMN_COUNT)
    rename_mapping = {

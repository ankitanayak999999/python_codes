...
# MISSING TRANSAFORMATION NAME FOR SOURCE ,TARGET ANF FILE ROWS  --> FIXED
# ADD: OBJECT_NAME (DB source/target = dsName.tableName.columnName; others blank)
...
def collect_source_target_refs(root):
    """
    Collect any Source/Target nodes with DF & Schema context.
    Returns dicts including:
      - df, schema, role, kind
      - outview (DIOutputView under Source)
      - fqname  (ds.owner.table)
      - ds/owner/table raw + canonical dsC/ownerC/tableC
    """
    ...
    for n in root.iter():
        ...
        # DIOutputView (Source label)
        outview = ""
        for ch in n.iter():
            if lower(strip_ns(getattr(ch, "tag", ""))) == "dioutputview":
                ov = (attrs_ci(ch).get("name") or "").strip()
                if ov:
                    outview = ov
                    break

        ds_raw    = a.get("datastorename") or a.get("datastore") or ""
        owner_raw = a.get("ownername")     or a.get("owner")     or a.get("schema") or ""
        tbl_raw   = a.get("tablename")     or a.get("table")     or a.get("name")   or ""
        fqname    = ".".join([p for p in (ds_raw, owner_raw, tbl_raw) if p])
        ...
        refs.append({
            ...
            "outview": outview,
            "fqname": fqname,
            "ds": ds_raw, "owner": owner_raw, "table": tbl_raw,
            "dsC": _canon(ds_raw),
            "ownerC": _canon(owner_raw),
            "tableC": _canon(tbl_raw),
        })
    return refs
...
def collect_db_st_rows(root, df_to_job, job_to_project, df_to_project, rows):
    ...
    for r in st_refs:
        ...
        for col in sorted(cols):
            # TRANSFORMATION_NAME per role
            trans_name = (r["outview"] or r["schema"]) if r["role"] == "SOURCE" else (r["fqname"] or r["schema"])
            # OBJECT_NAME = dsName.tableName.columnName   (exact spec)
            obj_name   = ".".join([p for p in (r["ds"], r["table"], col) if p])
            ...
            row = {
                ...
                "OBJECT_NAME": obj_name,          # <-- DB SOURCE/TARGET object identifier
                "TARGET_COLUMN": col,
                "SOURCE_COLUMNS": "",
                "MAPPING_TEXT": "",
            }
...
def collect_file_excel_st_rows(root, df_to_job, job_to_project, df_to_project, rows):
    ...
    for r in st_refs:
        ...
        row = {
            ...
            "TRANSFORMATION_TYPE": r["role"],
            "OBJECT_NAME": "",                  # no ds/table/column for file/excel
            "TARGET_COLUMN": "",
            "SOURCE_COLUMNS": "",
            "MAPPING_TEXT": "",
        }
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    if rows:
        df = pd.DataFrame(rows, columns=[
            ...
            "OBJECT_NAME","TARGET_COLUMN","SOURCE_COLUMNS","MAPPING_TEXT"
        ])
...
# ======================================================================
# 5) main() — SAME LAYOUT AS YOUR V5 (adds OBJECT_NAME + SOURCE_COL_COUNT)
# ======================================================================
def main():
    ...
    # RENAME / ORDER (kept your style, just added OBJECT_NAME)
    rename_mapping = {
    ...
    # SOURCE_COL_COUNT right after SOURCE_COLUMNS
    def _count_sources(s):
        ...
        return len(parts)
    final_df.insert(
        final_df.columns.get_loc("SOURCE_COLUMNS")+1,
        "SOURCE_COL_COUNT",
        final_df["SOURCE_COLUMNS"].apply(_count_sources)
    )

    # RECORD_KEY (unchanged)

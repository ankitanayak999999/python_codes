# 08/28 - ALL TRANSFORMATION WORKING; SOURCE COLUMNS ADDED; SOURCE/TARGET ROWS ADDED; FILE ROWS ADDED
# Updates:
# - TRANSFORMATION_NAME rules:
#     TRANSFORM -> DISchema name
#     SOURCE(DB) -> DIOutputView name
#     TARGET(DB) -> ds.table.column
# - OBJECT_NAME column:
#     DB SOURCE/TARGET -> ds.table.column
#     otherwise ""
# - SOURCE_COL_COUNT column (count of SOURCE_COLUMNS)
# - Column order kept; new columns appear before MAPPING_TEXT
#
...
def build_df_job_map(root):
    ...
    for n in root.iter():
        ...
        if t in JOB_TAGS:
            nm=_job_name_from_node(n);  if nm: jobs[nm]=n
        elif t in WF_TAGS:
            nm=(n.attrib.get("name") or n.attrib.get("displayName") or "").strip()
            if nm: wfs[nm]=n
    ...
    for j in jobs.keys():
        ...
        while stack:
            ...
            for nxt in edges.get(node,()):
                ...
                kind, nm = nxt
                if kind=="df":
                    real=df_canon.get(nm);  if real: reach.add(real)
                else:
                    stack.append(nxt)
...
def extract_ditable_catalog(root):
    """(DATASTORE, OWNER, TABLE) -> set(COLUMN_NAME) from global <DITable>."""
    by3 = defaultdict(set)
...
def collect_source_target_refs(root):
    """
    Collect Source/Target nodes with DF & Schema context.
    Adds 'outview' captured from DIOutputView under the same DISchema.
    """
    ...
    for n in root.iter():
        ...
        for anc in ancestors(n, pm, 200):
            ...
            if not schema_name and t=="dischema":
                ...
                schema_node=anc
        # find DIOutputView name within that schema (if present)
...
def _count_list(csv: str) -> int:
    if not csv: return 0
    return len([x for x in (s.strip() for s in csv.split(",")) if x])

# ======================================================================
# 3) HELPERS TO EMIT ROWS
# ======================================================================
    ...
    for e in root.iter():
        ...
        if tag == "dielement" and in_dataflow(e, pm):
            ...
            row = {
                ...
                "JOB_NAME": job,
                "DATAFLOW_NAME": df,
                "TRANSFORMATION_NAME": transform,          # TRANSFORM -> schema name
                "TRANSFORMATION_TYPE": "TRANSFORM",
                "TARGET_COLUMN": col_name,
                "OBJECT_NAME": "",                          # TRANSFORM -> empty
                "SOURCE_COLUMNS": src_cols,
                "SOURCE_COL_COUNT": _count_list(src_cols),
...
def collect_db_st_rows(root, df_to_job, job_to_project, df_to_project, rows):
    ...
    for r in st_refs:
        ...
        for col in sorted(cols):
            obj_name = f"{r['dsC']}.{r['tableC']}.{col}"  # ds.table.column

            # TRANSFORMATION_NAME rule for DB:
            if r["role"] == "SOURCE":
                tr_name = r["outview"] or r["schema"]  # prefer outview for SOURCE
            else:  # TARGET
                tr_name = obj_name                     # TARGET -> ds.table.column

            row = {
                "PROJECT_NAME": proj,
                "JOB_NAME": job,
                "DATAFLOW_NAME": r["df"],
                "TRANSFORMATION_NAME": tr_name,
...
def collect_file_excel_st_rows(root, df_to_job, job_to_project, df_to_project, rows):
    ...
    for r in st_refs:
        ...
        row = {
            ...
            "JOB_NAME": job,
            "DATAFLOW_NAME": r["df"],
            "TRANSFORMATION_NAME": r["schema"],   # kept as schema for non‑DB
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    # Maps to resolve PROJECT/JOB for a DF
    ...
    rows = []
    collect_transform_rows(root, pm, context_for, rows)
    collect_db_st_rows(root, df_to_job, job_to_project, df_to_project, rows)
    collect_file_excel_st_rows(root, df_to_job, job_to_project, df_to_project, rows)

    if rows:
        df = pd.DataFrame(rows, columns=[
            "PROJECT_NAME","JOB_NAME","DATAFLOW_NAME",
            "TRANSFORMATION_NAME","TRANSFORMATION_TYPE",
            "TARGET_COLUMN","OBJECT_NAME","SOURCE_COLUMNS","SOURCE_COL_COUNT","MAPPING_TEXT"
        ])
...
# ======================================================================
# 5) main() — SAME SHAPE AS YOUR V5 (only updated columns)
# ======================================================================
def main():
    ...
    # RENAME / ORDER — extended with OBJECT_NAME and SOURCE_COL_COUNT
    rename_mapping = {
    ...
    # RECORD_KEY (same style, now uses TARGET_COLUMN)

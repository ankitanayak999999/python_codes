...
# Step 1: Split SQL definitions into formatted .sql files
...
# Step 2: Scan .sql files for table and column matches
    ...
    for _, col_row in df_columns.iterrows():
        ...
        for view_file in os.listdir(view_sql_path):
            if view_file.endswith('.sql'):
                ...
                # Process columns only if table is found
                if tbl_count > 0:
                    ...
                    results.append({
                        ...
                        'Table Found (Y/N)': 'Y' if tbl_count > 0 else 'N',
                        'number of times table found': tbl_count,
                        'table found in line numbers': ', '.join(tbl_matching_lines),
                        'Column Found (Y/N)': 'Y' if col_count > 0 else 'N',

...
def process_mttask_files(src_path, tgt_path):
        ...
        for file in files:
            if file.lower() == "mttask" or file.lower() == "mttask.json":
                ...
                try:
                    ...
                    print(f"Error processing {file_path}: {def extract_all_zips(src_path):
    """
    Recursively extract all zip files from src_path and delete them.
    """
    # Step 1: Collect zip files
    zip_files = []
    for root, dirs, files in os.walk(src_path):
        for file in files:
            if file.lower().endswith(".zip"):
    ...
    # Step 2: Extract and delete each zip
    for zip_path in zip_files:
        try:
            ...
            # Step 3: Recursively check extracted content
            extract_all_zips(os.path.dirname(zip_path))
        except Exception as e:
            print(f"Error extracting {zip_path}: {e}")

...
def get_session_id(iics_base_url, 
                   iics_username, iics_password):
    """Get session ID from IDMC login API"""
    try:
        url = f"{iics_base_url}/ma/api/v2/user/login"
        payload = {
...
def get_table_lineage(asset_id, cdgc_base_url,
                      lineage_distance):
    """
...
def process_asset(row, cdgc_base_url, lineage_distance):
    ...
    try:
        ...
        response = get_table_lineage(
            asset_id, cdgc_base_url, lineage_distance)
...
def run_parallel(df_assets, cdgc_base_url,
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        futures = [
            executor.submit(
                process_asset, row,
                cdgc_base_url, lineage_distance)
...
if __name__ == "__main__":
    ...
    cdgc_base_url = config["urls"]["cdgc_api_url"]

    # ─── SCHEDULING ───────────────────────────────
    max_workers      = config[
        "cdgc_job_control"]["max_worker_table"]
    lineage_distance = config[
        "cdgc_job_control"]["lineage_distance"]

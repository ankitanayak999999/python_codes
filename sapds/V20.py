...
# lookup_ext variants + named kv (FUNCTION_CALL)
...
def parse_single_xml(xml_path: str):
    ...
    def add_db_source_target(e, role_hint):
        a=attrs_ci(e)
    ...
    def add_file_source_target(e, role_hint):
        # Attempt to get file info from various nodes/attrs
        a=attrs_ci(e)
        ds =(a.get("datastorename") or a.get("datastore") or a.get("name") or "FILE").strip()
        fmt=(a.get("file_format") or a.get("format") or a.get("type") or a.get("filetype") or "").strip()
        # file name may live in different places
        fname = (a.get("file_name") or a.get("filename") or a.get("path") or a.get("filepath") or "").strip()
        if not fname:
            # search children attributes that hold path/name
            txt=collect_text(e)
            m=re.search(r'(?:file[_\s]*name|path|filepath)\s*=\s*["\']([^"\']+)["\']', txt, re.I)
            if m: fname=m.group(1).strip()
        # display mapping: schema=fname, table=format
        schema = fname.split("/")[-1].split("\\")[-1] if fname else ""
        table  = fmt.upper() if fmt else "FILE"
        if ds and (schema or table):
            remember_display(ds, schema, table)
            proj,job,df=context_for(e)
            key=(proj,job,df,role_hint,_norm_key(ds),_norm_key(schema),_norm_key(table))
            file_source_target.add(key)
    ...
    for e in root.iter():
        ...
        if tag in ("didatabasetablesource","didatabasetabletarget"):
            add_db_source_target(e, "source" if "source" in tag else "target")

        # ------- file sources / targets (various shapes in exports) -------
        if tag in ("diflatfiledatastore","difileformat","diflatfilesource","diflatfiletarget","difilesource","difiletarget"):
            add_file_source_target(e, "source" if "source" in tag else ("target" if "target" in tag else "source"))

        # ------- lookup (column-level) via ui_mapping_text -------
        ...
        # ------- generic fallbacks (lookup / lookup_ext) -------
    ...
    for (proj,job,df,role,dsN,schN,tblN) in sorted(file_source_target):
        dsD,schD,tblD=nice_names(dsN,schN,tblN)
        rows.append(Row(proj or "", job or "", df or "", role,
                        dsD, schD, tblD, "", 0))
    ...
    # strict dedupe key + merge positions
    def nkey(r):
    ...
    agg=(df.groupby(
            ["__k__","project_name","job_name","dataflow_name","role","datastore","schema","table"],

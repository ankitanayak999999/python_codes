...
LOOKUP_ATTR_RE      = re.compile(r'tabledatastore\s*=\s*(?P<q1>[\'"]|&quot;)?(?P<ds>[^\'"&\s]+).*?tableowner\s*=\s*(?P<q2>[\'"]|&quot;)?(?P<own>[^\'"&\s]+).*?tablename\s*=\s*(?P<q3>[\'"]|&quot;)?(?P<tbl>[^\'"&\s]+)', re.I|re.S)
# named kv inside call text (order agnostic-ish)
LOOKUP_EXT_KV_RE    = re.compile(r'lookup_ext\s*\([^)]*(?:tableDatastore|tabledatastore)\s*=\s*([\'"]?)(?P<ds>[^\'",)\s]+)\1[^)]*(?:tableOwner|tableowner)\s*=\s*([\'"]?)(?P<own>[^\'",)\s]+)\3[^)]*(?:tableName|tablename)\s*=\s*([\'"]?)(?P<tbl>[^\'",)\s]+)\5', re.I|re.S)
...
def extract_lookup_from_call(text, is_ext=False):
    ...
    if is_ext:
        m3 = LOOKUP_EXT_KV_RE.search(text)
...
def index_udf_lookups(root):
    ...
    for n in root.iter():
        if lower(strip_ns(getattr(n,"tag",""))) in UDF_TAGS:
            ...
            uses=set()
            for pat in (LOOKUP_EXT_CALL_RE, LOOKUP_EXT_ARGS_RE, LOOKUP_EXT_KV_RE, LOOKUP_CALL_RE, LOOKUP_CALL_ARGS_RE):
    ...
    # regex to detect calls to any UDF: \b(NAME1|NAME2)\s*\(
    udf_names_regex = re.compile(r'\b(' + "|".join(names) + r')\s*\(', re.I) if names else None
    return udf_tables, udf_names_regex
...
def parse_single_xml(xml_path):
    ...
    # key=(proj,job,df,ds,sch,tbl) -> set of positions
    lookup_pos = defaultdict(list)   # for plain lookup (column-level): list of SchemaOut>>Col
    lookup_ext_pos = defaultdict(set) # for lookup_ext: set of SchemaOut or SchemaOut>>Col
    ...
    for e in root.iter():
        ...
        # collect a blob of meaningful text/attrs for generic scans & FUNCTION_CALL
        ...
        # ---------- column-level mapping text (lookup & UDF calls) ----------
        if tag=="diattribute" and lower(a.get("name",""))=="ui_mapping_text":
            ...
            # UDF calls (treat all lookups inside UDF as lookup_ext at this position)
        ...
        if tag in ("diexpression","diattribute"):
            ...
            if not expr: pass
                ...
                # UDF calls anywhere in the expr
        ...
        # ---------- FUNCTION_CALL (including kv/attrs) ----------
        if tag=="function_call":
            ...
            # function name itself might be a UDF; if so, add its internal lookups
            fn_name = (a.get("name") or "").strip().upper()
            if fn_name in (udf_tables.keys()):
                pos = f"{schema_out}>>{col}" if (schema_out and col) else schema_out
                if pos:
    ...
    # lookup_ext (even if no position resolved)
    ...
    # plus sources/targets
    out.extend(rows_src_tgt)

    # stable sort
    out.sort(key=lambda r:(r.project_name or "", r.job_name or "", r.dataflow_name or "", r.role, r.datastore or "", r.schema or "", r.table or "", r.lookup_position or ""))
    return out
...
def main():
    ...
    out_base = os.path.splitext(xml_path)[0] + "_lineage_udf"

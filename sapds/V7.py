...
def parse_single_xml(xml_path: str):
    ...
    seen_ext_keys = set()              # avoid dup when both FUNCTION_CALL and DIExpression exist
    custom_sql_rows = []
    ...
    for e in root.iter():
        ...
        if tag in ("difilesource","difiletarget"):
            ...
            # schema = file name, table = formatName
            schema_val = fn
            table_val  = fmt
            remember_display(ds, schema_val, table_val)
            role="source" if tag=="difilesource" else "target"
            key=(proj,job,df,role,_norm_key(ds),_norm_key(schema_val),_norm_key(table_val))
        ...
        # -------- Custom SQL transform (as SOURCE rows) ----------
        if tag == "ditransformcall":
            # gather SQL bits if present under this transform
            # 1) identify transform/schema/display name to show as "position"
            tf_disp = ""
            for ch in e.iter():
                if lower(strip_ns(getattr(ch,"tag",""))) == "diattribute" and lower(ch.attrib.get("name","")) == "ui_display_name":
                    tf_disp = (ch.attrib.get("value") or "").strip() or tf_disp
                if lower(strip_ns(getattr(ch,"tag",""))) == "dischema":
                    nm = (ch.attrib.get("name") or "").strip()
                    if nm: tf_disp = tf_disp or nm
            # 2) database_datastore at transform level (if present)
            db_ds = ""
            for ch in e.iter():
                if lower(strip_ns(getattr(ch,"tag",""))) == "diattribute" and lower(ch.attrib.get("name","")) == "database_datastore":
                    db_ds = (ch.attrib.get("value") or "").strip() or db_ds
            # 3) collect SQL text
            sql_texts=[]
            for ch in e.iter():
                if lower(strip_ns(getattr(ch,"tag",""))) in ("sqltext","sql_text"):
                    sql_texts.append(collect_text(ch))
                elif lower(strip_ns(getattr(ch,"tag","")))=="diattribute" and lower(ch.attrib.get("name",""))=="sql_text":
                    sql_texts.append((ch.attrib.get("value") or ""))
            if sql_texts:
                ...
                custom_sql_rows.append((
                    proj, job, df, "source", ds, sch, tbls,
                    (tf_disp or "SQL"), len(dedupe(tables)), quote_sql_exact(full_sql)
                ))
        ...
        if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
            ...
            if dsx and tbx and schema_out:
                ...
                seen_ext_keys.add(k)  # authoritative capture
        ...
        if tag in ("diexpression","diattribute","function_call"):
            ...
            # lookup_ext fallback ONLY if not already captured
    ...
    Row = namedtuple("Row", [
        ...
        "in_transformation_used_count","Custom_SQL"
    ])
    rows=[]
    ...
    # custom SQL
    for (proj,job,df,role,ds,sch,tbls,pos,count,sqltxt) in custom_sql_rows:
        dsD,schD,tblD = nice_names(_norm_key(ds), _norm_key(sch), _norm_key(tbls))
        rows.append(Row(proj or "", job or "", df or "", role,
                        dsD, schD, tblD, pos, count, sqltxt))
    ...
    df["in_transformation_used_count"]=df["transformation_position"].apply(
        lambda x: len([p for p in dedupe([pp.strip() for pp in str(x).split(",")]) if p and ">>" in p])
        if "lookup" in ",".join(df.columns) else
        df.get("in_transformation_used_count", 0)
    )
    # For non-lookup rows (source/target/custom sql) where this count was 0, recompute from position if present
    df.loc[df["in_transformation_used_count"].isna(), "in_transformation_used_count"] = 0
...
def main():
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # EDIT THESE PATHS:
    xml_path = r"C:\path\to\export.xml"                         # your SAP DS export
    out_xlsx = r"C:\path\to\xml_lineage_output.xlsx"            # output Excel
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

    df = parse_single_xml(xml_path)

    # enforce template column order & fill missing
    cols = [
        "project_name","job_name","dataflow_name","role",

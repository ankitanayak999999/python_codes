...
Record = namedtuple("Record", [
    ...
    "lookup_used_count"     # # of UNIQUE positions
])
...
def _job_name_from_node(job_node) -> str:
    ...
    for ch in job_node.iter():
        if lower(strip_ns(getattr(ch, "tag", ""))) == "diattribute" and lower(ch.attrib.get("name", "")) == "job_name":
            ...
            if v:
                return v
    # else fallback to DIJob @name/displayName
    return (job_node.attrib.get("name") or job_node.attrib.get("displayName") or "").strip()

def build_df_job_map(root):
    """Top-down mapping: Dataflow → Job (works even if DIJob is not an ancestor)."""
    df_names = collect_df_names(root)
    df_job = {}
    for jn in root.iter():
        ...
        if not job_name:
            continue
        # (1) dataflows nested inside the job
        for df_node in jn.iter():
            if lower(strip_ns(getattr(df_node, "tag", ""))) in DF_TAGS:
                nm = (df_node.attrib.get("name") or df_node.attrib.get("displayName") or "").strip()
                if nm:
                    df_job.setdefault(nm, job_name)
        # (2) callsteps referencing dataflows/workflows that point to dataflows
        for call in jn.iter():
        ...
        # (3) any attribute text in the job subtree that exactly matches a DF name
        for desc in jn.iter():
            if not hasattr(desc, "attrib"):
                continue
            for _, v in desc.attrib.items():
...
def parse_single_xml(xml_path: str):
    ...
    rows = []  # will hold source/target rows
    seen = set()
    ...
    for elem in root.iter():
        ...
        # collect blob text (for FUNCTION_CALL fallbacks)
        ...
        if tag_l == "diattribute" and lower(a.get("name", "")) == "ui_mapping_text":
            ...
            # UDFs that contain lookups → treat like lookup_ext at that position
            if txt:
                m_udf = UDF_NAME_RE.search(txt)
                if m_udf:
                    udf_name = m_udf.group(1).strip().upper()
                    # build UDF index once, then use it:
                    # (already in udf_tables)
                    if udf_name in udf_tables:
                        ...
                        for (dsu, schu, tblu) in udf_tables[udf_name]:
                            ...
                            _ = lookup_ext_positions[key]   # ensure key
                            if pos:
        ...
        if tag_l in ("diexpression", "diattribute"):
            ...
            if "lookup_ext(" in expr_l:
                ...
                # parse straight from expr and emit (supports dot & comma styles)
        ...
        if tag_l == "function_call":
            ...
            if ds and tbl:
                ...
                # if this FUNCTION_CALL lives under a DIAssignmentStep we saw earlier, apply that schema context
                step = nearest_ancestor(elem, parent_map, "diassignmentstep")

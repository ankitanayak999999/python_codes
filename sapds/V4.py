...
def collect_text(n):
    ...
    if (getattr(n,"text",None)): parts.append(n.text)
    for c in list(n):
        parts.append(collect_text(c))
        if (getattr(c,"tail",None)): parts.append(c.tail)
...
def parse_single_xml(xml_path: str):
    ...
    # --- 1) index lookups defined INSIDE each DIScriptFunction ---
    # function_name -> { "lookup": set((dsN, schN, tblN)), "lookup_ext": set((dsN, schN, tblN)) }
    func_lookup = defaultdict(lambda: {"lookup": set(), "lookup_ext": set()})

    for fnode in root.iter():
        if lower(strip_ns(getattr(fnode,"tag",""))) not in SCRIPT_FUNC: continue
        fname = (attrs_ci(fnode).get("name") or "").strip()
        if not fname: continue
        # scan the function body for lookups
        for n in fnode.iter():
            if not isinstance(n.tag, str): continue
            tag = lower(strip_ns(n.tag))
            a   = attrs_ci(n)
            blob = " ".join([f'{k}="{v}"' for k,v in a.items()]) + " " + collect_text(n)

            # lookup_ext — prefer named KV via FUNCTION_CALL, else parse blob
            if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
                ...
                if dsx and tbx:
                    k=(_norm_key(dsx),_norm_key(schx),_norm_key(tbx))
                    func_lookup[fname]["lookup_ext"].add(k)
                    remember_display(dsx,schx,tbx)
            elif HAS_LOOKUP_EXT.search(blob):
                dsx,schx,tbx = extract_lookup_from_call(blob, is_ext=True)
                if dsx and tbx:
                    k=(_norm_key(dsx),_norm_key(schx),_norm_key(tbx))
                    func_lookup[fname]["lookup_ext"].add(k)
            ...
            # lookup (rare inside functions, but support anyway)
            if tag=="function_call" and lower(a.get("name",""))=="lookup":
                dsl,schl,tbl = extract_lookup_from_call(blob, is_ext=False)
                if dsl and tbl:
                    k=(_norm_key(dsl),_norm_key(schl),_norm_key(tbl))
                    func_lookup[fname]["lookup"].add(k)
                    remember_display(dsl,schl,tbl)
            elif HAS_LOOKUP.search(blob):
                dsl,schl,tbl = extract_lookup_from_call(blob, is_ext=False)
                if dsl and tbl:
                    k=(_norm_key(dsl),_norm_key(schl),_norm_key(tbl))
                    func_lookup[fname]["lookup"].add(k)
    ...
    # helpers to resolve pretty names back from normalized triplets
    def pretty_triplet(dsN, schN, tblN):
        key = (dsN, schN, tblN)
        return (display_ds[key].get(dsN), display_sch[key].get(schN), display_tbl[key].get(tblN))

    # --- 2) walk dataflows and collect: sources, targets, direct lookups, and FUNCTION lookups ---
    source_target=set()
    lookup_pos    = defaultdict(list)  # (proj,job,df,dsN,schN,tblN,"lookup") -> [schema>>col,...]
    lookup_ext_pos= defaultdict(list)  # (proj,job,df,dsN,schN,tblN,"lookup_ext") -> [schema>>col,...]
    ...
    # simple set of all function names to match calls by text
    all_func_names = {fn for fn in func_lookup.keys()}
    # pattern to spot any of those function names in text (whole word-ish)
    func_call_pat = re.compile(r'\b(' + "|".join([re.escape(fn) for fn in all_func_names]) + r')\b', re.I) if all_func_names else None
    ...
    for e in root.iter():
        ...
        # Column UI mapping (captures direct lookup + function calls for column)
        if tag=="diattribute" and lower(a.get("name",""))=="ui_mapping_text":
            ...
            # 2b) function used in column: attach its internal lookups
            if func_call_pat and func_call_pat.search(txt):
                for m in func_call_pat.finditer(txt):
                    fn_used = m.group(1)
                    if fn_used in func_lookup and schema_out and col:
                        for (dsN, schN, tblN) in func_lookup[fn_used]["lookup_ext"]:
                            lookup_ext_pos[(proj,job,df,dsN,schN,tblN)].append(f"{schema_out}>>{col}")
                        for (dsN, schN, tblN) in func_lookup[fn_used]["lookup"]:
                            lookup_pos[(proj,job,df,dsN,schN,tblN)].append(f"{schema_out}>>{col}")

        # lookup_ext directly in the DF (FUNCTION_CALL preferred)
        if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
            proj,job,df=context_for(e)
            schema_out=schema_out_from_DISchema(e, pm, cur_schema)
            dsx,schx,tbx = extract_lookup_from_call(" ".join([f'{k}="{v}"' for k,v in a.items()]), is_ext=True)
            ...
            if dsx and tbx and schema_out:
                remember_display(dsx,schx,tbx)
                lookup_ext_pos[(proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))].append(schema_out)

        # generic fallbacks (rare, but keep)
        if tag in ("diexpression","function_call"):
    ...
    for (proj,job,df,dsN,schN,tblN), positions in lookup_pos.items():
        ...
        dsD,schD,tblD = pretty_triplet(dsN,schN,tblN)
    ...
    df["in_transf_used_count"]=df["lookup_position"].apply(lambda x: len([p for p in dedupe([pp.strip() for pp in str(x).split(",")]) if p]))
...
def main():
    ...
    out_xlsx = r"C:\path\to\xml_lineage_output.xlsx"         # output Excel
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

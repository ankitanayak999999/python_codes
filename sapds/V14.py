...
def parse_single_xml(xml_path: str):
    ...
    lookup_ext_transforms = defaultdict(set)    # (job,dflow,ds,sch,tbl) -> { "Schema", "Schema>>Col", ... }
    ...
    for elem in root.iter():
        ...
        if tag_l == "function_call":
            ...
            if ds and tbl:
                ...
                # if inside a DIAssignmentStep we saw earlier, use that info
                step = nearest_ancestor(elem, parent_map, "diassignmentstep")
    ...
    for (job, dflow, ds, sch, tbl), posset in lookup_ext_transforms.items():

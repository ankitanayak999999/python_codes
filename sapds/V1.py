...
def normalize_candidates(val: str):
    """Return candidate tokens we’ll try to match to DIDataFlow names."""
    if not val: return []
...
def build_df_job_map(root):
    ...
    df_job = {}
    for jn in root.iter():
        t = lower(strip_ns(getattr(jn, "tag", "")))
        if t not in JOB_TAGS: continue
...
def build_df_project_map(root):
    ...
    df_proj = {}
    for pn in root.iter():
        t = lower(strip_ns(getattr(pn, "tag", "")))
        if t not in PROJECT_TAGS: continue
        proj_name = (pn.attrib.get("name") or pn.attrib.get("displayName") or "").strip()
        if not proj_name: continue
        # look for dataflow references inside the project subtree
        for desc in pn.iter():
            if not hasattr(desc, "attrib"): 
                continue
            for _, v in desc.attrib.items():
        ...
        # also: if the project nests dataflows directly
...
def parse_single_xml(xml_path: str):
    ...
    lookup_ext_transforms = defaultdict(set)    # (proj,job,df,ds,sch,tbl) -> { "Schema", "Schema>>Col", ... }
    ...
    for elem in root.iter():
        ...
        if tag_l in ("didatabasetablesource","didatabasetabletarget"):
            ...
            # fill from maps if missing; if still missing, leave blank (as requested)
            job0 = job0 or (df_job_map.get(dflow0) or "")
            proj0 = proj0 or (df_proj_map.get(dflow0) or "")
            key = (proj0, job0, dflow0, "source" if "source" in tag_l else "target", ds, sch, tbl, "", "", 0)
            if key not in seen:
                seen.add(key)
                rows.append(Record(proj0, job0, dflow0, "source" if "source" in tag_l else "target", ds, sch, tbl, "", "", 0))
            continue
        ...
        if tag_l == "diattribute" and lower(a.get("name","")) == "ui_mapping_text":
            ...
            # UDF call → attach UDF's internal lookup tables as lookup_ext at this schema/column
            if txt:
                ...
                if m_udf:
                    ...
                    if udf_name in udf_tables:
                        ...
                        for (dsu, schu, tblu) in udf_tables[udf_name]:
                            ...
                            if pos: lookup_ext_transforms[base_key].add(pos)
        ...
        if tag_l in ("diexpression","diattribute"):
            ...
            if "lookup_ext(" in lower(expr_txt):
                ...
                if step is not None:
                    ...
                    proj0 = proj0 or (df_proj_map.get(dflow0) or "")
                    schema_out = schema_out_from_DISchema(elem, parent_map, current_schema_out)
                    pending_ext_by_step[id(step)] = {"schema_out": schema_out, "project": proj0, "job": job0, "dflow": dflow0}
        ...
        if tag_l == "function_call":
            ...
            if ds and tbl:
                ...
                # if inside a DIAssignmentStep we saw earlier, prefer that info
                step = nearest_ancestor(elem, parent_map, "diassignmentstep")
        ...
        if "lookup(" in lower(blob) or "lookup_ext(" in lower(blob)):
            ...
            # transform/column-level lookup_ext(...)
    ...
    # lookup_ext rows (positions may be "Schema>>Col" or just "Schema")
    for (proj, job, df, ds, sch, tbl), posset in lookup_ext_transforms.items():
    ...
    rows.sort(key=lambda r: (r.project_name, r.job_name, r.dataflow_name, r.role, r.datastore, r.schema, r.table, r.lookup_position))
    return rows

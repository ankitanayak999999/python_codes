...
Record = namedtuple(
    ...
    [
        ...
        # --- v17 patch additions ---
        "datastore_details","sql_pick_method",
    ],
)
...
def build_df_job_map(root):
    ...
    if len(jobs)==1:
        ...
        for d in collect_df_names(root): df_job.setdefault(d, only)
    return df_job
...
# -------- robust UI display name for custom SQL --------

def find_ui_display_name(e, pm):
    # nearest DISchema first
    nearest_schema = None
...
# -------- datastore details (for custom SQL rows) --------

def collect_datastore_details_from_ancestors(e, pm):
    """
    Walk up ancestors and pick the NEAREST <DIDatabaseDatastore> block
    (or similarly named) and flatten its interesting attributes.
    Return a single comma-separated string like:
      name=ODS_DS, type=Oracle, version=11g, database=ORCL, host=db01, ...
    If nothing found, return "".
    """
    interesting_keys = {
        "name","displayname","databasetype","database_type","type",
        "databaseversion","database_version","version",
        "database","server","host","port","user","owner",
        "unicode","encoding","category","database_category"
    }
    for up in ancestors(e, pm, 400):
        t = lower(strip_ns(getattr(up, "tag", "")))
        if t in ("didatabasedatastore","databasedatastore","datastore","didatastore"):
            a = attrs_ci(up)
            bits=[]
            for k,v in a.items():
                ...
                if key in interesting_keys:
                    ...
                    if val:
                        bits.append(f"{k}={val}")
            if bits:
                # stable order: sort by key name so repeats are consistent
                bits_sorted = sorted(bits, key=lambda s: s.split("=",1)[0].lower())
                return ", ".join(bits_sorted)
    return ""
...
def extract_tables_from_sql(sql_text: str):
    """
    Use sqlglot; if it fails, fall back to regex.
    """
    if not sql_text:
        return []
    try:
        ...
        tables = set()
        # collect CTE names to avoid counting them
        cte_names = set()
        for expr in expressions:
            for cte in expr.find_all(sqlglot.exp.CTE):
                alias = cte.args.get("alias")
                if isinstance(alias, sqlglot.exp.TableAlias) and alias.name:
                    cte_names.add(alias.name.upper())
        for expr in expressions:
            for t in expr.find_all(sqlglot.exp.Table):
                t_name = t.name
                t_db   = t.args.get("db") or ""
                if not t_db and t_name and t_name.upper() in cte_names:
                    continue
                tables.add(f"{t_db}.{t_name}" if t_db else t_name)
        table_list=[t.strip() for t in tables if t and str(t).strip()]
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    # NOTE: include source_line + (patch fields) on each row
    ...
    # NEW: active datastores per DF (to match custom SQL datastore)
    df_active_datastores = defaultdict(set)  # df -> set of ds names seen in sources/targets
    ...
    for e in root.iter():
        ...
        # 1) DB sources/targets (inside DF)
        if tag in ("didatabasetablesource","didatabasetabletarget") and in_dataflow(e, pm):
            ...
            if ds and tbl:
                ...
                if df and ds: df_active_datastores[df].add(ds)

        # 2) FILE / EXCEL sources/targets (inside DF)
        if tag in ("difilesource","difiletarget","diexcelsource","diexceltarget") and in_dataflow(e, pm):
            ...
            for ch in e.iter():
                if lower(strip_ns(getattr(ch, "tag", ""))) in ("dioutputview","outputview"):
                    ...
                    if outview_name:
                        break
            fname = (a.get("filename") or a.get("name") or a.get("datasetname") or "").strip()
            if outview_name:
            ...
            key = (proj, job, df, role, _norm_key(ds), _norm_key(sch), _norm_key(tbl), line_no(e))
            source_target.add(key)
            if df and ds and ds not in ("FILE","EXCEL"):  # keep DF datastore set focused on DB-like stores
                df_active_datastores[df].add(ds)
        ...
        if tag in ("sqltext","sqltexts","diquery","ditransformcall") and in_dataflow(e, pm):
            ...
            if sql_text:
                ...
                # UI display name -> SCHEMA and TRANSFORMATION_POSITION
                ...
                # datastore for this SQL (if present)
                ds_for_sql=""
                ...
                # NEW: collect nearest <DIDatabaseDatastore> details
                ds_details = collect_datastore_details_from_ancestors(e, pm)

                # NEW: pick method hint (matched to DF's active datastores?)
                ds_matched = 1 if (df and ds_for_sql in df_active_datastores.get(df, set())) else 0
                pick_method = "DS_MATCHED" if ds_matched else "FIRST_SQL"
                ...
                sql_rows.append(Record(
                    ...
                    disp_name,            # transformation_position
                    len(tables),
                    '"' + (sql_text.replace('"','""')) + '"',
                    line_no(e),
                    ds_details,           # <- DATASTORE_DETAILS
                    pick_method,          # <- SQL_PICK_METHOD
                ))

        # 4) LOOKUPS — FUNCTION_CALL inside DF (unchanged)
        if tag=="function_call" and in_dataflow(e, pm):
            ...
            if not (ds and tbl):
                missing_lookup.append(Record(
                    ...
                    pos, 1, "", ln,
                    "", ""   # ds details, pick method not applicable
                ))
            else:
                remember_display(ds, sch, tbl)
    ...
    # 5) expand external function lookups ONLY for DFs that call them

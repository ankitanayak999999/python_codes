...
def find_context(elem, parent_map):
    """Return (project, job, dataflow) with job name from DIAttribute if present."""
    proj = job = dflow = ""
    for anc in ancestors(elem, parent_map):
        ...
        if t in ("dijob","dibatchjob","job","batch_job") and not job:
            # default to DIJob @name; prefer <DIAttribute name="job_name" value="...">
            job_name = nm
            for ch in anc.iter():
...
# comma-arg style (job-level exports)
...
LOOKUP_EXT_CALL_ARGS_RE = re.compile(
    r'lookup_ext\s*\(\s*[\'"]?\s*([A-Za-z0-9_]+)\s*[\'"]?\s*,\s*[\'"]?\s*([A-Za-z0-9_]+)\s*[\'"]?\s*,\s*[\'"]?\s*([A-Za-z0-9_]+)\s*[\'"]?',
    re.IGNORECASE
)

def extract_lookup_from_call(text, is_ext=False):
    """Return (datastore, owner/schema, table) from either dot or comma arg styles."""
    if not text:
        return ("", "", "")
...
# attributes blob form (FUNCTION_CALL with attributes)
...
def build_df_job_map(root):
    """Map dataflow name -> job name (job_name attribute preferred)."""
    df_names = collect_df_names(root)
    df_job = {}
    df_score = {}

    def get_job_name(node, fallback):
        for ch in node.iter():
            if lower(strip_ns(getattr(ch, "tag", ""))) == "diattribute" and lower(ch.attrib.get("name", "")) == "job_name":
    ...
    for jn in root.iter():
        tag = lower(strip_ns(getattr(jn, "tag", "")))
        if tag not in JOB_TAGS:
            continue

        base_name = (jn.attrib.get("name") or jn.attrib.get("displayName") or "").strip()
        if not base_name:
            continue

        score = 2  # Job priority
        name_for_map = get_job_name(jn, base_name)
                ...
                for cand in normalize_candidates(v):
                    for df in df_names:
                        if cand.lower() == df.lower():
                            if df not in df_job or score > df_score.get(df, 0):
                                df_job[df] = name_for_map
                                df_score[df] = score
    return df_job
...
def parse_single_xml(xml_path: str):
    ...
    for elem in root.iter():
        ...
        # streaming context (no 'continue' so we still scan this node)
        ...
        if tag_l in ("dijob","dibatchjob","job","batch_job"):
        ...
        # Column-level lookup within ui_mapping_text (captures lookup and UDFs)
        if tag_l == "diattribute" and lower(a.get("name","")) == "ui_mapping_text":
            ...
            # UDF call that itself contains lookups → treat as lookup_ext at this position
            if txt:
        ...
        if tag_l == "function_call":
            ...
            # fallback: attributes embedded in the blob (quoted)
            ...
            # last fallback: parse function call text (dot or comma-arg)
            if not (ds and tbl):

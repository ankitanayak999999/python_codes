...
# >>> NEW: index DIScriptFunction definitions that contain lookup/lookup_ext
def index_discript_functions(root):
    """
    Returns: dict { UPPER_FUNC_NAME: [(role, ds, sch, tbl), ...] }
    Scans inside <DIScriptFunction> for either:
      - <FUNCTION_CALL name="lookup|lookup_ext" ...>
      - or textual patterns inside its body.
    """
    idx = defaultdict(list)
    for fn in root.iter():
        if lower(strip_ns(getattr(fn, "tag", ""))) != "discriptfunction":
            continue
        fname = (getattr(fn, "attrib", {}).get("name") or getattr(fn, "attrib", {}).get("displayName") or "").strip()
        if not fname:
            continue
        key = canon(fname)
        ...
        for fc in fn.iter():
            ...
            ds, sch, tbl = extract_lookup_from_call(" ".join([f'{k}="{v}"' for k, v in a.items()]), is_ext=is_ext)
            if not ds:
                ds, sch, tbl = extract_lookup_from_call(collect_text(fc), is_ext=is_ext)
            if ds and tbl:
                idx[key].append(("lookup_ext" if is_ext else "lookup", ds, sch, tbl))

        # 2) text fallback
        blob = collect_text(fn)
        if HAS_LOOKUP_EXT.search(blob) or HAS_LOOKUP.search(blob):
            # try both; whichever matches yields a triple
            ds, sch, tbl = extract_lookup_from_call(blob, is_ext=True)
            if ds and tbl:
                idx[key].append(("lookup_ext", ds, sch, tbl))
            ds, sch, tbl = extract_lookup_from_call(blob, is_ext=False)
            if ds and tbl:
                idx[key].append(("lookup", ds, sch, tbl))
...
def parse_single_xml(xml_path: str):
    ...
    # >>> NEW: build function index once
    fn_index = index_discript_functions(root)  # {FUNCNAME: [(role,ds,sch,tbl), ...]}
    # <<< NEW
    ...
    source_target=set()
    lookup_pos    = defaultdict(list)  # schema>>column (for lookup)
    lookup_ext_pos= defaultdict(list)  # >>> NEW: make ext column-level too
    # seen_ext_keys removed to not suppress distinct column positions
    ...
    for e in root.iter():
        ...
        if tag=="diattribute" and lower(a.get("name",""))=="ui_mapping_text":
            ...
            # >>> NEW: if ui_mapping_text calls a user-defined DIScriptFunction, expand it
            txt_low = lower(txt)
            if "(" in txt_low:  # cheap guard
                for fname_key, calls in fn_index.items():
                    # match function name by word boundary + "("
                    if re.search(rf'\b{re.escape(fname_key)}\s*\(', txt_low):
                        proj,job,df=context_for(e)
                        schema_out=schema_out_from_DISchema(e, pm, cur_schema)
                        col=find_output_column(e, pm)
                        for role, dsx, schx, tbx in calls:
                            if not (dsx and tbx): continue
                            remember_display(dsx, schx, tbx)
                            if role == "lookup_ext":
                                lookup_ext_pos[(proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))]\
                                    .append(f"{schema_out}>>{col}")
                            else:
                                lookup_pos[(proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))]\
                                    .append(f"{schema_out}>>{col}")
            # <<< NEW
        ...
        if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
            ...
            if dsx and tbx and schema_out:
                ...
                remember_display(dsx,schx,tbx)
                # >>> CHANGED: store column-level position for lookup_ext too
                lookup_ext_pos[k].append(f"{schema_out}>>{col}")
                # <<< CHANGED

        # >>> NEW: FUNCTION_CALL that invokes a user-defined function by name
        if tag=="function_call":
            nm = lower(a.get("name",""))
            if nm and nm not in ("lookup","lookup_ext"):
                key_name = canon(nm)
                if key_name in fn_index:
                    proj,job,df=context_for(e)
                    schema_out=schema_out_from_DISchema(e, pm, cur_schema)
                    col=find_output_column(e, pm)
                    for role, dsx, schx, tbx in fn_index[key_name]:
                        if not (dsx and tbx): continue
                        remember_display(dsx, schx, tbx)
                        k = (proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))
                        if role == "lookup_ext":
        ...
        if tag in ("diexpression","diattribute","function_call"):
            ...
            # lookup_ext fallback (allow additional positions)
            ...
            # >>> NEW: text that calls user-defined function names inside expressions
            blob_low = lower(blob)
            if "(" in blob_low:
                for fname_key, calls in fn_index.items():
                    if re.search(rf'\b{re.escape(fname_key)}\s*\(', blob_low):
                        for role, dsx, schx, tbx in calls:
                            if not (dsx and tbx): continue
                            remember_display(dsx, schx, tbx)
                            k = (proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))
                            ...
                            if role == "lookup_ext":
                                lookup_ext_pos[k].append(pos)
                            else:
                                lookup_pos[k].append(pos)
            # <<< NEW
...
def main():
    ...
    out_xlsx = r"C:\path\to\export_xml_lineage_final.xlsx"  # output Excel
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

    df = parse_single_xml(xml_path)

    # enforce template columns & order

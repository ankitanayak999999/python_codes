...
def schema_out_from_DISchema(e, pm, fallback=""):
    """Nearest DISchema name, prefer non-'Join'."""
    best=None; join=None
...
def parse_single_xml(xml_path: str):
    ...
    source_target=set()
    lookup_pos    = defaultdict(list)  # schema>>column (column-level lookup)
    lookup_ext_pos= defaultdict(set)   # schema only (transform-level lookup_ext)
    ...
    for e in root.iter():
        ...
        # column-level UI mapping → lookup (not lookup_ext)
        ...
        # FUNCTION_CALL: prefer named KV for lookup_ext
        if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
            ...
            if dsx and tbx and schema_out:
                ...
                lookup_ext_pos[(proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))].add(schema_out)
            continue  # avoid double capture from DIExpression

        # generic expressions / attributes fallback
        if tag in ("diexpression","diattribute","function_call"):
            ...
            # lookup_ext (fallback)
            if HAS_LOOKUP_EXT.search(blob) and not (tag=="function_call" and lower(a.get("name",""))=="lookup_ext"):
    ...
    for (proj,job,df,dsN,schN,tblN), positions in lookup_pos.items():
        ...
        rows.append(RecordRow(proj or "", job or "", df or "", "lookup",
    ...
    for (proj,job,df,dsN,schN,tblN), posset in lookup_ext_pos.items():
        ...
        dsD,schD,tblD=nice_names(dsN,schN,tblN)
        rows.append(RecordRow(proj or "", job or "", df or "", "lookup_ext",
                              dsD, schD, tblD, ", ".join(uniq), len(uniq)))

    for (proj,job,df,role,dsN,schN,tblN) in sorted(source_target):
        dsD,schD,tblD=nice_names(dsN,schN,tblN)
        rows.append(RecordRow(proj or "", job or "", df or "", role,
                              dsD, schD, tblD, "", 0))

...
SQL_TABLE_RE = re.compile(r'\b(?:from|join)\s+([A-Za-z0-9_\."]+)', re.I)
...
def build_job_to_project_map_from_summary(root):
    """
    Some multi-project exports place DIProject + DIJobRef blocks
    near the end. Build job->project from those.
    """
    j2p = {}
    for p in root.iter():
...
def build_df_project_map(root):
    """
    Maps dataflow -> project.
    Handles projects that actually contain their DFs and trailing
    <DIProject><DIJobRef/>…</DIProject> summaries.
    """
        ...
        if lower(strip_ns(getattr(p,"tag",""))) in PROJECT_TAGS:
            ...
            for d in p.iter():
                if lower(strip_ns(getattr(d,"tag",""))) in DF_TAGS:
                    df = (d.attrib.get("name") or d.attrib.get("displayName") or "").strip()
                    if df: df_proj.setdefault(df, proj)

    # (2) if only one project total, assign it
    projects = [ (p.attrib.get("name") or p.attrib.get("displayName") or "").strip()
                 for p in root.iter()
                 if lower(strip_ns(getattr(p,"tag",""))) in PROJECT_TAGS ]
...
def parse_single_xml(xml_path: str):
    ...
    job_to_project = build_job_to_project_map_from_summary(root)
    ...
    def remember_display(ds, sch, tbl):
        ds=_strip_wrappers(ds).strip(); sch=_strip_wrappers(sch).strip(); tbl=_strip_wrappers(tbl).strip()
    ...
    seen_ext_keys  = set()              # to avoid dup when FUNCTION_CALL + fallback both hit
    source_target  = set()              # (proj,job,df,role,ds,sch,tbl)
    sql_rows       = []                 # list of Record-like tuples for custom SQL

    # for external function attribution
    function_bodies = collect_function_bodies(root={})  # placeholder to satisfy linter
    # robust function body collection:
    function_bodies = {}
    FUNCTION_DEF_TAGS = {
    ...
    for node in root.iter():
        ...
        name = (getattr(node,"attrib",{}).get("name") or getattr(node,"attrib",{}).get("displayName") or "").strip()
        if not name: continue
        if tag in FUNCTION_DEF_TAGS:
            function_bodies[name] = collect_text(node); continue
        # fallback heuristic: any named node that clearly contains lookup text
        txt = collect_text(node)
        if ("lookup(" in txt.lower() or "lookup_ext(" in txt.lower()):
            function_bodies.setdefault(name, txt)
    ...
    for e in root.iter():
        ...
        if tag in ("sqltext","sqltexts","diquery","ditransformcall"):
            ...
            if sql_text:
                proj,job,df=context_for(e)
                # display-ish name for the transform position
                disp_name=""
                ...
                # datastore near SQL (if any), else DS_SQL
                ds_for_sql=""
                for up in ancestors(e, pm, 12):
                    for ch in up.iter():
                ...
                sql_rows.append((
                    proj,job,df,"source",
                    ds_for_sql,"CUSTOM_SQL",table_csv or "SQL_TEXT",
                    disp_name or "SQL",
                    len(tables),
                    quote_sql(sql_text),
                ))
        ...
        if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
            ...
            if not dsx:
                dsx,schx,txb = extract_lookup_from_call(collect_text(e), is_ext=True)
                if not dsx:
                    schx, txb = "", ""
            if dsx and (tbx or txb) and schema_out:
                tb_final = tbx or txb
                k=(proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tb_final))
                remember_display(dsx,schx,tb_final)
                lookup_ext_pos[k].add(schema_out)
                seen_ext_keys.add(k)

        # ------- fallback blobs + record function positions -------
        if tag in ("diexpression","diattribute","function_call"):
            ...
            # record function calls for external expansion
            if schema_out:
            ...
            if tag=="function_call":
                ...
                if callee and callee.lower() not in ("lookup","lookup_ext") and schema_out:
                    pos=f"{schema_out}>>{col}" if col else schema_out
                    df_func_positions[df][callee].add(pos)

            # fallback lookup_ext (only if not already captured via FUNCTION_CALL)
            if HAS_LOOKUP_EXT.search(blob) and schema_out:
            ...
            # fallback lookup
            if (HAS_LOOKUP.search(blob) or (tag=="function_call" and lower(a.get("name",""))=="lookup")) and schema_out and col:
    ...
    for (proj,job,df,dsN,schN,tblN), positions in lookup_pos.items():
        ...
        rows.append((proj or "", job or "", df or "", "lookup",
    ...
    for (proj,job,df,dsN,schN,tblN), posset in lookup_ext_pos.items():
        ...
        dsD,schD,tblD=nice_names(dsN,schN,tblN)
        rows.append((proj or "", job or "", df or "", "lookup_ext",
                     dsD, schD, tblD, ", ".join(uniq), len(uniq), ""))

    # sources / targets
    for (proj,job,df,role,dsN,schN,tblN) in sorted(source_target):
        dsD,schD,tblD=nice_names(dsN,schN,tblN)
        rows.append((proj or "", job or "", df or "", role,
                     dsD, schD, tblD, "", 0, ""))
    ...
    # backfill project_name via job map and DF map
    def fill_proj(row):
    ...
    # strict de-dupe with merged positions
    def nkey(r):
    ...
    df=(df.groupby(
            ["__k__","project_name","job_name","dataflow_name","role","datastore","schema","table","custom_sql_text"],
            dropna=False, as_index=False)
         .agg({"transformation_position": lambda s: ", ".join(sorted(dedupe(list(s)))),
...
# ------------------------ function body collection (helper) ------------------------

def collect_function_bodies(root):
    # (Kept for API completeness; actual collection in parse_single_xml for robustness)
    return {}

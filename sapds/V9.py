...
# split DS.OWNER.TABLE if we accidentally captured all in ds
def fix_packed_ds(ds, sch, tbl):
    ds = (ds or "").strip()
    sch = (sch or "").strip()
    tbl = (tbl or "").strip()
    if ds and (not sch or not tbl) and "." in ds:
        parts = [p for p in DOT_NORMALIZE.sub(".", ds).split(".") if p]
        if len(parts) >= 3:
            ds, sch, tbl = parts[0], parts[1], parts[2]
    # scrub policy/cache tokens
    if _is_cache_or_policy(sch): sch = ""
    if _is_cache_or_policy(tbl): tbl = ""
    return ds, sch, tbl

def valid_triplet_after_fix(ds, sch, tbl):
    return _valid_triplet(ds, sch, tbl)
...
def build_df_project_map(root):
    # map DF -> project using explicit containment under DIProject
    df_proj={}
    projects=[]
    for p in root.iter():
    ...
    for proj_name, pnode in projects:
        for d in pnode.iter():
            if lower(strip_ns(getattr(d,"tag",""))) in DF_TAGS:
                ...
                if dn: df_proj.setdefault(dn, proj_name)
    # if only one project in file, default any remaining DFs to it
...
SQL_TABLE_RE = re.compile(r'\bfrom\s+([A-Z0-9_\.]+)', re.I)
...
def parse_single_xml(xml_path: str):
    ...
    seen_lookup_keys=set()             # authoritative FUNCTION_CALL lookup
    seen_lookupext_keys=set()          # authoritative FUNCTION_CALL lookup_ext
    sql_rows=[]                        # custom SQL captures
    ...
    for e in root.iter():
        ...
        if tag in ("difilesource","difilet arget","difiletarget"):  # also guard typographical variants
            proj,job,df=context_for(e)
            fmt=a.get("formatname") or ""
            fname=a.get("filename") or ""
            ds = a.get("datastorename") or "FILE"
            sch = fmt
            tbl = os.path.basename(fname) if fname else fname
            if ds or sch or tbl:
                remember_display(ds,sch,tbl)
                role="source" if "source" in tag else "target"
                key=(proj,job,df,role,_norm_key(ds),_norm_key(sch),_norm_key(tbl))
        ...
        if tag in ("ditransformcall","diquery"):
            # capture SQL name (from DISchema name or UI display)
            proj,job,df=context_for(e)
            # find children carrying SQL text
            sql_texts=[]
            for ch in e.iter():
                if lower(strip_ns(getattr(ch,"tag",""))) in ("sqltexts","sqltext","sql_text"):
                    sql_texts.append(collect_text(ch))
            sql_text = " ".join(sql_texts).strip()
            if sql_text:
                # pull table tokens
                tables = [m.group(1).strip() for m in SQL_TABLE_RE.finditer(sql_text)]
                tables = dedupe([t for t in tables if t])
                schema = "CUSTOM_SQL"
                tbl    = ", ".join(tables) if tables else ""
                ds     = a.get("datastorename") or a.get("database_datastore") or ""
                ds     = ds or "DS_ODS"  # benign default if absent
                remember_display(ds, schema, tbl or "SQL")
                sql_rows.append((
                    proj or "", job or "", df or "",
                    "source", ds, schema, tbl,
                    "", 0, '"' + sql_text.replace('"','""') + '"'
                ))
        ...
        if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
            ...
            dsx,schx,tbx = fix_packed_ds(dsx, schx, tbx)
            if dsx and tbx and schema_out and valid_triplet_after_fix(dsx,schx,tbx):
        ...
        # ---------------- lookup (FUNCTION_CALL authoritative) ----------------
        if tag=="function_call" and lower(a.get("name",""))=="lookup":
            ...
            dsl,schl,tbl = fix_packed_ds(dsl,schl,tbl)

            if dsl and tbl and schema_out and col and valid_triplet_after_fix(dsl,schl,tbl):
        ...
        # ---------------- fallback parsing (only if not already captured) ----------------
    ...
    # file sources / targets
    for (proj,job,df,role,dsN,schN,tblN) in sorted(file_st):
        dsD,schD,tblD=nice_names(dsN,schN,tblN)
        rows.append(Row(proj or "", job or "", df or "", role,
                        dsD, schD, tblD, "", 0, ""))
    ...
    # strict dedupe by normalized key and merge positions & SQL (comma-join unique)
    def nkey(r):
...
def main():
    ...
    xml_path = r"C:\path\to\export.xml"                     # project/job/df export (supports multi-project)
    out_xlsx = r"C:\path\to\xml_lineage_output_v8.xlsx"     # output workbook
    # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

    df = parse_single_xml(xml_path)

    # enforce template columns & order (keeps older order, adds custom_sql)
    cols = [
        "project_name","job_name","dataflow_name","role",

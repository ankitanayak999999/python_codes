...
# lookup_ext patterns (for FUNCTION body parsing / fallback)
...
def extract_all_lookups(text: str, is_ext: bool):
    """Used when parsing user function bodies. Strictly validated."""
    found=[]
    ...
    if is_ext:
        for mkv in LOOKUP_EXT_NAMED_KV_RE.finditer(t):
            ds, sch, tb = (mkv.group("ds") or "").strip(), (mkv.group("own") or "").strip(), (mkv.group("tbl") or "").strip()
            if _valid_triplet(ds, sch, tb): found.append((ds, sch, tb))

    BRE = BRACED_TRIPLE_EXT if is_ext else BRACED_TRIPLE
    for m in BRE.finditer(t):
        ds, sch, tb = m.group(1).strip(), m.group(2).strip(), m.group(3).strip()
        if _valid_triplet(ds, sch, tb): found.append((ds, sch, tb))
...
# ------------------------ main parser (V8 base + lookup_ext dedupe by position) ------------------------
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    lookup_ext_pos  = defaultdict(set)   # (proj,job,df,ds,sch,tbl) -> {"Schema", ...}  (positions are schema-only)
    direct_ext_pos  = defaultdict(set)   # positions captured from direct FUNCTION_CALL
    source_target   = set()
    ...
    for e in root.iter():
        ...
        # ------- lookup_ext (FUNCTION_CALL: primary capture) -------
        if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
            proj,job,df=context_for(e)
            schema_out=schema_out_from_DISchema(e, pm, cur_schema)
            # Read attributes only (authoritative)
            ...
            if dsx and tbx and schema_out:
                ...
                # record direct positions so we can skip duplicates from fallback/expansion
                direct_ext_pos[k].add(schema_out)

        # ------- record function positions (for external UDF mapping) + optional fallback -------
        if tag in ("diexpression","diattribute","function_call"):
            ...
            # record any function calls (to attribute external UDF bodies back to DF)
            if schema_out:
            ...
            # ---- OPTIONAL fallback for lookup_ext: capture ONLY if its position differs from direct one ----
            if HAS_LOOKUP_EXT.search(blob):
                dsx,schx,tbx=extract_lookup_from_call(blob, is_ext=True)
                if dsx and tbx and schema_out:
                    k = (proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))
                    # position for lookup_ext is schema-only (no column)
                    if schema_out not in direct_ext_pos.get(k, set()):
                        remember_display(dsx,schx,tbx)
                        lookup_ext_pos[k].add(schema_out)

            # fallback lookup (non-ext) stays (column-level)
    ...
    # ---- expand external function lookups into DF collectors, skipping direct-duplicates by position ----
    for df_name, fn_map in df_func_positions.items():
        ...
        for fn_key, positions in fn_map.items():
            ...
            # lookup (non-ext) in function body -> column positions

...
# -------- function-discovery to catch external lookups (custom functions) --------
...
def build_job_to_project_map(root):
    """Map Job -> Project via trailing <DIProject><DIJobRef name=.../> blocks."""
    j2p={}
    for p in root.iter():
...
def build_df_project_map(root):
    """DF -> Project by containment (fallbacks for single-project)."""
...
def collect_function_bodies(root):
    ...
    for node in root.iter():
        ...
        if tag in FUNCTION_DEF_TAGS:
            bodies[name] = collect_text(node)
            continue
        # heuristic fallback: any named node that contains lookup text
        txt = collect_text(node)
        if ("lookup(" in txt.lower() or "lookup_ext(" in txt.lower()):
            bodies.setdefault(name, txt)
    return bodies

# ------------------------ main parser (V6 + SQL + multi-project) ------------------------
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    # function bodies (for external lookup expansion)
    function_bodies = collect_function_bodies(root)
    ...
    seen_ext_keys  = set()
    source_target  = set()
    sql_rows       = []  # tuples shaped like Record
    ...
    agg = (
        df.groupby(
            ...
            {"transformation_position": lambda s: ", ".join(sorted(dedupe(list(s)))),

...
# ------------------------ function body collection (robust) ------------------------
...
Record = namedtuple(
    ...
    [
        ...
        "transformation_position","tranformation_useges_count","custom_sql",
    ],
)
...
def parse_single_xml(xml_path: str):
    ...
    # external function usage mapping
    ...
    for e in root.iter():
        ...
        if tag in ("sqltext","sqltexts","diquery","ditransformcall"):
            ...
            if sql_text:
                proj, job, df = context_for(e)
                # a display-ish name for the transform position
                disp_name = ""
    ...
    # compute the renamed count column
    df["tranformation_useges_count"] = df["transformation_position"].apply(

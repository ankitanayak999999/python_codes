...
def parse_single_xml(xml_path: str):
    ...
    custom_sql_rows = []               # collected SQL transform rows
    ...
    for e in root.iter():
        ...
        if tag in ("difilesource","difiletarget"):
            ...
            table  = (a.get("filename") or a.get("name") or "").strip()
        ...
        if tag=="ditransformcall":
            ...
            if sql_chunks:
                ...
                custom_sql_rows.append( (proj,job,df,"source", "DS_SQL", schema, table, pos, 1, f"\"{sql_text.strip()}\"") )
    ...
    # strict dedupe by normalized key and merge positions/sql
    def nkey(r):
    ...
    def merge_pos(s):
        return ", ".join(sorted(dedupe([x for x in (", ".join([str(y) for y in s])).split(",")])))

    agg=(df.groupby(["__k__","project_name","job_name","dataflow_name","role",
                     "datastore","schema","table"], dropna=False, as_index=False)
            .agg({
                "transformation_position": merge_pos,
                "custom_sql": lambda s: " ".join([x for x in dedupe([str(v).strip() for v in s if str(v).strip()])])
            }))

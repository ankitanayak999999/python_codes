...
"""
SAP Data Services XML lineage extractor (V8)
- Base: V6 (robust lookup + lookup_ext, including external DIScriptFunction calls invoked inside a DF)
- Adds: V7 custom SQL (schema=CUSTOM_SQL, table=list of tables from SQL, full SQL quoted)
- Adds: Multi-project mapping (DIProject → DIJobRef → Job; call graph Job/Workflow → Dataflow)
- Supports: DB and File sources/targets
- Output de-duped and with separated datastore/schema/table columns
Columns:
...
def wrap_double_quotes(s: str) -> str:
    """Surround text with double quotes, escape internal double quotes, flatten newlines."""
    if s is None: return ""
...
def extract_lookup_from_call(text: str, is_ext: bool = False):
    ...
    if is_ext:
        ...
        if mkv:
            ds, sch, tbl = parse_clean_triplet(mkv.group("ds"), mkv.group("own"), mkv.group("tbl"))
            if _valid_triplet(ds, sch, tbl): return ds, sch, tbl
...
def build_project_job_map(root):
    """Map job name -> project name via DIProject/DIJobRef blocks (handles multi-project exports)."""
    job_to_project={}
    for p in root.iter():
...
def parse_single_xml(xml_path: str):
    ...
    for e in root.iter():
        ...
        # rolling context
        if tag in PROJECT_TAGS: cur_project=(a.get("name") or a.get("displayname") or cur_project).strip()
        ...
        if tag in ("difilesource","difiletarget"):
            ...
            if schema or table:
                ...
                proj,job,df=context_for(e)
                role="source" if "source" in tag else "target"
                key=(proj,job,df,role,_norm_key(ds),_norm_key(schema),_norm_key(table))
                source_target.add(key)

        # --------- Authoritative FUNCTION_CALL lookup/lookup_ext ---------
        if tag=="function_call":
            ...
            if nm in ("lookup","lookup_ext"):
                ...
                if not dsx:
                    dsx,schx,tbx = extract_lookup_from_call(collect_text(e), is_ext=(nm=="lookup_ext"))
                if dsx and tbx and schema_out:
                    remember_display(dsx,schx,tbx)
                    key=(proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))
                    if nm=="lookup_ext":
                        lookup_ext_pos[key].add(schema_out); seen_lookupext_keys.add(key)
                    else:
        ...
        # --------- DIScriptFunction call-sites & fallback scanning ---------
        if tag in ("diattribute","diexpression"):
            ...
            # attach DIScriptFunction internals (external functions referenced inside DF)
            for fn_name, packs in script_fn_index.items():
                if fn_name and fn_name in blob:
                    for (ds,sch,tb) in packs.get("lookup_ext",()):
                        ...
                        k=(proj,job,df,_norm_key(ds),_norm_key(sch),_norm_key(tb))
            ...
            # fallback inline parsing only if not already captured
        ...
        if tag=="ditransformcall":
            sql_chunks=[]; disp_name=""; ds_for_sql=""
            ds_for_sql = a.get("database_datastore","").strip()
            for ch in e.iter():
                ...
                if t2=="diattribute" and lower(ch.attrib.get("name",""))=="database_datastore":
                    ds_for_sql = ch.attrib.get("value","").strip() or ds_for_sql
                if t2=="sqltext":
                    for st in ch.iter():
            ...
            if sql_chunks:
                ...
                custom_sql_rows.append( (proj,job,df,"custom_sql", (ds_for_sql or ""), schema, table, pos, 1, wrap_double_quotes(sql_text)) )
    ...
    for c in ("datastore","schema","table","project_name","job_name","dataflow_name","transformation_position","custom_sql"):

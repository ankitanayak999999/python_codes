...
- Preserves V6 lookup/lookup_ext behavior (captures external DIScriptFunction calls invoked inside a DF)
- Adds V7 custom SQL capture (schema=CUSTOM_SQL, table=list of tables, custom_sql quoted)
- Handles multi-project exports (DIProject → DIJobRef → Job; call graph job/workflow → DF)
- Supports DB and File sources/targets
- De-dupes cleanly and keeps datastore/schema/table separated
Output columns:
...
def wrap_double_quotes(s: str) -> str:
    """Surround text with double quotes and escape internal quotes; flatten newlines."""
    if s is None: return ""
...
def parse_single_xml(xml_path: str):
    ...
    for e in root.iter():
        ...
        if tag in ("diattribute","diexpression"):
            ...
            # attach DIScriptFunction internals
            ...
            # inline fallback (only if not already captured)

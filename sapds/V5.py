...
DF_TAGS       = ("didataflow","dataflow","didataflow")
...
def parse_single_xml(xml_path: str):
    ...
    for e in root.iter():
        ...
        if tag in FILE_SOURCE_TAGS or tag in FILE_TARGET_TAGS:
            ds =(a.get("datastorename") or a.get("datastore") or "FILE").strip() or "FILE"
            sch=(a.get("formatname") or "").strip()      # schema = formatName
            tbl=(a.get("filename") or "").strip()        # table  = filename
            if not tbl:
                # last resort: use "name" if filename missing
                tbl = (a.get("name") or "").strip()
            if sch or tbl:
                remember_display(ds, sch, tbl)
                proj,job,df=context_for(e)
                role="source" if tag in FILE_SOURCE_TAGS else "target"
                key=(proj,job,df,role,_norm_key(ds),_norm_key(sch),_norm_key(tbl))
                source_target.add(key)

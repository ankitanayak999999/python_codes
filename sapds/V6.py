...
class NameBag:
    ...
    def _score(self, s):
        ...
        return (1 if " " in s else 0, 1 if s == _pretty(s) else 0, len(s))
    def add(self, s):
        s = _pretty(s)
        if not s: return
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    # ----- PASS 1: index all DIScriptFunction lookups -----
    script_nodes = {}
    for n in root.iter():
        if lower(strip_ns(getattr(n,"tag",""))) == "discriptfunction":
            nm = (n.attrib.get("name") or n.attrib.get("displayName") or "").strip()
            if nm: script_nodes[canon(nm)] = n

    func_ext_map = defaultdict(set)   # FUNC -> {(ds,sch,tbl)}
    func_std_map = defaultdict(set)   # FUNC -> {(ds,sch,tbl)}

    for cname, node in script_nodes.items():
        for e in node.iter():
            if not isinstance(e.tag,str): continue
            tag = lower(strip_ns(e.tag)); a = attrs_ci(e)
            blob = " ".join([f'{k}="{v}"' for k,v in a.items()]) + " " + collect_text(e)

            if (tag=="function_call" and lower(a.get("name",""))=="lookup_ext") or HAS_LOOKUP_EXT.search(blob):
                dsx,schx,tbx = extract_lookup_from_call(blob, is_ext=True)
                if dsx and tbx:
                    remember_display(dsx,schx,tbx)
                    func_ext_map[cname].add((_norm_key(dsx),_norm_key(schx),_norm_key(tbx)))

            if (tag=="function_call" and lower(a.get("name",""))=="lookup") or HAS_LOOKUP.search(blob):
                dsl,schl,tbl = extract_lookup_from_call(blob, is_ext=False)
                if dsl and tbl:
                    remember_display(dsl,schl,tbl)
                    func_std_map[cname].add((_norm_key(dsl),_norm_key(schl),_norm_key(tbl)))

    # Build regex for function invocation in text (ui_mapping_text)
    func_name_re = None
    if script_nodes:
        pat = "|".join([re.escape(_strip_wrappers(k)) for k in set([ (n.attrib.get("name") or n.attrib.get("displayName") or "") for n in script_nodes.values() ]) if k])
        if pat:
    ...
    for e in root.iter():
        ...
        # FILE sources/targets (formatName->schema, filename->table, datastore=FILE)
        if tag in ("difilesource","difiletarget"):
            fmt = (a.get("formatname") or "").strip()
            fn  = (a.get("filename")   or "").strip()
            ds, sch, tbl = ("FILE", fmt, fn)
            proj,job,df=context_for(e)
            role="source" if "source" in tag else "target"
            remember_display(ds, sch, tbl)
            key=(proj,job,df,role,_norm_key(ds),_norm_key(sch),_norm_key(tbl))
        ...
        # Column-level UI mapping text (lookup and function invocations)
        if tag=="diattribute" and lower(a.get("name",""))=="ui_mapping_text":
            ...
            # function invocation → pull lookups from script map
            if func_name_re and schema_out:
                for m in func_name_re.finditer(txt):
                    fname = m.group(1)
                    cname = canon(fname)
                    # script lookup_ext (schema-level)
                    for dsN,schN,tblN in func_ext_map.get(cname, ()):
                        lookup_ext_pos[(proj,job,df,dsN,schN,tblN)].add(schema_out)
                    # script plain lookup (column-level) → attach to this column
                    if col:
                        for dsN,schN,tblN in func_std_map.get(cname, ()):
                            lookup_pos[(proj,job,df,dsN,schN,tblN)].append(f"{schema_out}>>{col}")

        # lookup_ext within DF (FUNCTION_CALL preferred)
        ...
        # generic fallback (lookup & lookup_ext) inside DF
    ...
    # dedupe by normalized identity and merge positions
    def nkey(r):

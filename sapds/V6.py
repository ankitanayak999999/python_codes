...
"""
SAP Data Services export.xml lineage extractor
- Keeps V4 behavior: captures lookup + lookup_ext from ui_mapping_text AND from DIScriptFunction() bodies,
  linking them back to the DIDataflow / column that calls the function.
- Adds V5 behavior: file sources/targets (DIFileSource/DIFileTarget) with formatName -> table, filename -> schema,
  stable project/job/dataflow mapping, and robust de-dup/cleaning.

Output columns (one worksheet 'lineage'):
...
def strip_ns(tag):  # remove XML namespace
    return re.sub(r"^\{.*\}", "", tag) if isinstance(tag, str) else ""

def lower(s): return (s or "").strip().lower()
def attrs_ci(e): return { (k or "").lower(): (v or "") for k, v in getattr(e, "attrib", {}).items() }
...
class NameBag:
    """keep the 'nicest' seen variant for display"""
...
# lookup_ext() patterns (incl. FUNCTION_CALL KV args)
...
def extract_lookup_from_call(text: str, is_ext: bool = False):
    ...
    if is_ext:
        mkv = LOOKUP_EXT_NAMED_KV_RE.search(t)  # most reliable for FUNCTION_CALL
...
def build_df_job_map(root):
    ...
    for cs in root.iter():
        ...
        # find the calling ancestor (job or workflow)
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    # --- collected facts ---
    source_target=set()
    lookup_pos    = defaultdict(list)  # key -> list of "schema>>column"
    lookup_ext_pos= defaultdict(list)  # key -> list of "schema>>column"
    seen_ext_keys = set()              # to avoid duplicates when both FUNCTION_CALL and fallback exist

    # for DIScriptFunction: function_name => { (dsN, schN, tblN) }
    function_lookup_ext = defaultdict(set)

    cur_proj=cur_job=cur_df=cur_schema=""; last_job=""

    def context_for(e):
        """figure out project/job/df for any node, with fallbacks to maps"""
        proj=job=df=""
    ...
    # ---------- first pass: harvest lookups & sources/targets ----------
    for e in root.iter():
        ...
        if tag in ("difilesource","difiletarget"):
            # Use formatName as table, filename as schema (your request)
            fmt = (a.get("formatname") or "").strip()
            fname = (a.get("filename") or "").strip()
            # derive a shorter display for filename
            fname_disp = os.path.basename(_pretty(fname)) or fname or "FILE"
        ...
        # --- lookup from ui_mapping_text (column level) ---
        ...
        # --- lookup_ext captured at FUNCTION_CALL (authoritative) ---
        if tag=="function_call" and lower(a.get("name",""))=="lookup_ext":
            ...
            if dsx and tbx and schema_out:
                ...
                # we capture schema out here; column will be added if the call is inside an output element
                col=find_output_column(e, pm)
                key=(proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))
                if col:
                    lookup_ext_pos[key].append(f"{schema_out}>>{col}")
                else:
                    lookup_ext_pos[key].append(schema_out)  # rare fallback
                seen_ext_keys.add((canon(proj),canon(job),canon(df),key))

        # --- fallback parsing for lookup/lookup_ext anywhere in blobs ---
        if tag in ("diexpression","diattribute","function_call"):
            ...
            # lookup_ext fallback only if not already captured via FUNCTION_CALL for same key
            if HAS_LOOKUP_EXT.search(blob):
                dsx,schx,tbx=extract_lookup_from_call(blob, is_ext=True)
                if dsx and tbx and schema_out:
                    key=(proj,job,df,_norm_key(dsx),_norm_key(schx),_norm_key(tbx))
                    marker=(canon(proj),canon(job),canon(df),key)
                    if marker not in seen_ext_keys:
                        remember_display(dsx,schx,tbx)
                        if col:
                            lookup_ext_pos[key].append(f"{schema_out}>>{col}")
                        else:
                            lookup_ext_pos[key].append(schema_out)

        # --- harvest lookup_ext hidden inside DIScriptFunction bodies (V4 behavior) ---
        if tag == "discriptfunction":
            fn_name = (a.get("name") or a.get("displayname") or "").strip()
            if not fn_name: continue
            blob = " ".join([f'{k}="{v}"' for k,v in a.items()]) + " " + collect_text(e)
            if HAS_LOOKUP_EXT.search(blob):
                # collect all triples appearing inside the function
                cursor = 0
                while True:
                    m = LOOKUP_EXT_NAMED_KV_RE.search(blob, cursor) or \
                        BRACED_TRIPLE_EXT.search(blob, cursor) or \
                        LOOKUP_EXT_CALL_RE.search(blob, cursor) or \
                        LOOKUP_EXT_ARGS_RE.search(blob, cursor)
                    if not m: break
                    dsx,schx,tbx = extract_lookup_from_call(blob[m.start():m.end()], is_ext=True)
                    cursor = m.end()
                    if dsx and tbx:
                        function_lookup_ext[fn_name].add((_norm_key(dsx), _norm_key(schx), _norm_key(tbx)))
                        remember_display(dsx, schx, tbx)

    # ---------- second pass: link DIScriptFunction calls to columns that use them ----------
    for e in root.iter():
        if not isinstance(e.tag, str): continue
        tag=lower(strip_ns(e.tag)); a=attrs_ci(e)
        ...
        if tag=="diattribute" and lower(a.get("name",""))=="ui_mapping_text":
            txt=a.get("value") or e.text or ""
            if not txt: continue
            # find any script function names present in this ui_mapping_text
            for fn_name, triples in function_lookup_ext.items():
                if fn_name and fn_name in txt:
                    proj,job,df = None, None, None
                    proj, job, df = None, None, None
                    proj,job,df = "", "", ""
                    proj,job,df = "", "", ""
                    proj,job,df = "", "", ""
                    proj,job,df = next(iter([(x,y,z) for (x,y,z) in [(_,_,_)]]), ("","",""))  # (no-op just to keep linters calm)

                    proj,job,df=context_for(e)
                    schema_out=schema_out_from_DISchema(e, pm, "")
                    col=find_output_column(e, pm)
                    if not (schema_out and col): continue
                    for dsN, schN, tblN in triples:
                        key=(proj,job,df,dsN,schN,tblN)
                        lookup_ext_pos[key].append(f"{schema_out}>>{col}")

    # ---------- finalize rows ----------
    def nice(dsN, schN, tblN):
        k=(dsN, schN, tblN)
    ...
    for (proj,job,df,dsN,schN,tblN), positions in lookup_pos.items():
        ...
        dsD,schD,tblD=nice(dsN,schN,tblN)
        rows.append(Row(proj or "", job or "", df or "", "lookup",
                        dsD, schD, tblD, ", ".join(uniq), len(uniq)))

    # lookup_ext
    for (proj,job,df,dsN,schN,tblN), positions in lookup_ext_pos.items():
        uniq=sorted(dedupe([p.strip() for p in positions if p and p.strip()]))
        dsD,schD,tblD=nice(dsN,schN,tblN)
    ...
    agg=(df.groupby(["__k__","project_name","job_name","dataflow_name","role",
            ...
            .agg({"lookup_position": lambda s: ", ".join(sorted(dedupe([x.strip() for x in s if str(x).strip()])))}))
    df=agg.drop(columns=["__k__"])
    df["in_transf_used_count"]=df["lookup_position"].apply(

...
def attrs_ci(e):
    return {k.lower(): (v or "") for k, v in (getattr(e, "attrib", {}) or {}).items()}
...
def parse_single_xml(xml_path: str) -> pd.DataFrame:
    ...
    df_to_job      = build_df_job_map(root)  # infer job for DFs via CallSteps
    ...
    # collectors ---------------------------------------------------
    # source/target: keep earliest line number per unique key
    source_target  = {}  # key -> min_line
    sql_rows       = []  # list of Record tuples
    # key -> position -> set(lines)
    lookup_pos     = defaultdict(lambda: defaultdict(set))
    lookup_ext_pos = defaultdict(lambda: defaultdict(set))
    missing_lookup = []  # rows with missing DS/Owner/Table

    df_context        = {}  # df -> (proj,job)
    # df -> canon(fn) -> list of (position, callsite_line)
    df_func_positions = defaultdict(lambda: defaultdict(list))
    ...
    for e in root.iter():
        ...
        if tag in ("didatabasetablesource","didatabasetabletarget"):
            ...
            if ds and tbl:
                ...
                ln=line_no(e)
                prev=source_target.get(key)
                source_target[key] = ln if prev is None else min(prev, ln)
        ...
        # ---- 3) CUSTOM SQL (same as V9, plus ui_display_name & line)
        if tag in ("sqltext","sqltexts","diquery","ditransformcall"):
            ...
            if sql_text:
                proj,job,df=context_for(e)
                # position name prefers ui_display_name
                disp_name=""
                ...
                table_csv=", ".join(tables) if len(tables)>0 else "SQL_TEXT"
                ds_for_sql=""
                for up in ancestors(e, pm, 12):
                    # examine children explicitly; no truthiness check
                    kids=list(up)
                    if len(kids)==0:
                        continue
                    for ch in kids:
        ...
        if tag=="function_call":
            ...
            if cal not in ("lookup","lookup_ext"):
                ...
                if schema_out:
                    ...
                    ln_call = line_no(e)
                    if fn_key:
                        df_func_positions[df][fn_key].append((pos, ln_call))
                # not a lookup; continue
                continue
    ...
    for df_name, fn_map in df_func_positions.items():
        ...
        for fn_key, pos_calls in fn_map.items():
            # only if the function actually contains lookups
            body_lookups = fn_lookup_calls.get(fn_key, [])
            if len(body_lookups)==0:
                continue
            for (pos, ln_call) in pos_calls:
                for (role, ds, sch, tbl) in body_lookups:
                    if not (ds and tbl):
                        missing_lookup.append(Record(
                    ...
                    if role == "lookup_ext":
                        lookup_ext_pos[key][pos].add(ln_call)
                    else:
                        lookup_pos[key][pos].add(ln_call)
    ...
    for (proj,job,df,dsN,schN,tblN), posmap in lookup_pos.items():
        ...
        rows.append(Record(proj, job, df, "lookup", dsD, schD, tblD,
                           ", ".join(positions), len(positions), "", (min(lines) if len(lines)>0 else -1)))
    ...
    # sources / targets (+ line numbers)
    for (proj,job,df,role,dsN,schN,tblN), ln in sorted(source_target.items()):
        dsD,schD,tblD=nice_names(dsN,schN,tblN)
        rows.append(Record(proj, job, df, role, dsD, schD, tblD, "", 0, "", ln if ln is not None else -1))
...
def main():
    ...
    out_xlsx = r"C:\Users\raksahu\Downloads\python\input\output_v11_afs.xlsx"

...
def schema_out_from_DISchema(elem, parent_map, fallback="") -> str:
    """Nearest <DISchema name='...'> up the tree; else fallback."""
    for anc in ancestors(elem, parent_map):
...
def find_output_column(elem, parent_map) -> str:
    """Nearest <DIElement name='...'> up the tree."""
...
def build_workflow_df_map(root, df_names):
    """Workflow name -> set of dataflow names it calls (directly)."""
...
def _job_name_from_node(job_node) -> str:
    # Prefer <DIAttribute name="job_name" value="...">
...
def build_df_job_map(root):
    """
    Dataflow name -> job name
    Handles:
      - jobs that directly contain/call dataflows
      - jobs that call workflows which call dataflows
    ...
    for jn in root.iter():
        ...
        # (2) Callsteps: if they call workflows, expand via wf_df; if they name DFs directly, map directly
        for call in jn.iter():
            if lower(strip_ns(getattr(call, "tag", ""))) in CALLSTEP_TAGS:
                for _, v in getattr(call, "attrib", {}).items():
                    for cand in _normalize_candidates(v):
        ...
        # (3) Any attribute in job subtree that equals a DF name
        for desc in jn.iter():
            if not hasattr(desc, "attrib"):
                continue
            for _, v in desc.attrib.items():
...
# comma-arg style DS, SCHEMA, TABLE (allow optional { } and quotes)
...
def extract_lookup_from_call(text: str, is_ext: bool = False):
    """Return (datastore, owner/schema, table) from dot or comma-arg styles."""
    if not text:
        return ("", "", "")
    ...
    # comma-arg style (with optional braces/quotes/whitespace/newlines)
...
def parse_single_xml(xml_path: str):
    ...
    current_dflow   = ""
    current_schema  = ""
    last_seen_job   = ""   # helps project-level exports where job is separate
    pending_ext_by_step = {}  # id(step) -> context dict

    def context_for(elem):
        """Ancestors + DF→Job map + last_seen_job fallback."""
        proj = job = df = ""
        for anc in ancestors(elem, parent_map):
    ...
    for elem in root.iter():
        ...
        if tag_l == "diattribute" and lower(a.get("name", "")) == "ui_mapping_text":
            ...
            # UDFs that contain lookups → treat as lookup_ext at the same position
            if txt:
        ...
        if tag_l in ("diexpression", "diattribute"):
            ...
            if "lookup_ext(" in expr_l:
                # remember step context (for position resolution)
        ...
        if tag_l == "function_call":
            ...
            # attributes embedded in blob text
            ...
            if ds and tbl:
                ...
                # if under a DIAssignmentStep we saw earlier, apply that schema context
                step = nearest_ancestor(elem, parent_map, "diassignmentstep")
        ...
        # This scans the assembled blob from ANY node to ensure we don't miss calls that
        # aren't wrapped as FUNCTION_CALL or DIExpression in job-level exports.
    ...
    # lookup_ext rows (emit even if no position resolved → count 0)
...
def main():
    ...
    out_base = os.path.splitext(xml_path)[0] + "_lineage_v4"

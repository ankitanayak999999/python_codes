...
def parse_single_xml(xml_path: str):
    ...
    for elem in root.iter():
        ...
        # streaming context (no 'continue' here so we still scan this node)
        ...
        if tag_l in ("didatabasetablesource","didatabasetabletarget"):
            ...
            # do not 'continue'; still allow blob scan for odd exports
        ...
        if tag_l == "function_call":
            ...
            # last fallback: parse function call text (lookup / lookup_ext)
            if not (ds and tbl):
        ...
        if "lookup(" in lower(blob) or "lookup_ext(" in lower(blob):
            ...
            # 2b) lookup_ext(...) anywhere (treat as transform/column-level)

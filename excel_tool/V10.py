...
def get_user_inputs_dynamic(fields):
    ...
    def submit():
        ...
        for i, entry in enumerate(entries):
            val = entry.get() if hasattr(entry, "get") else entry.get()
        ...
        result = [e.get() if hasattr(e, "get") else e.get() for e in entries]
        root.destroy()
    ...
    for i, field in enumerate(fields):
        ...
        if ftype == "dropdown":
            ...
            for j, option in enumerate(field.get("options", [])):
                tk.Radiobutton(root, text=option, variable=var, value=option).grid(row=i, column=1+j, sticky="w")
            entries.append(var)
        ...
        warning.grid(row=i, column=6, sticky="w")
        warnings.append(warning)

    # Focus and submit bindings
    entries[0].focus()
    ...
    root.mainloop()
    return [e.get() if hasattr(e, "get") else e.get() for e in from gui_engine import get_user_inputs_dynamic

fields = [
    {"label": "Enter Username", "type": "text", "required": True},
    {"label": "Select Report File", "type": "file", "required": True},
    {"label": "Output Format", "type": "radio", "required": True, "options": ["PDF", "Excel", "HTML"]},
    {"label": "Department", "type": "dropdown", "required": False, "options": ["Finance", "IT", "HR"]}
]

inputs = get_user_inputs_dynamic(fields)

if inputs:
    for f, val in zip(fields, inputs):
        print(f"{f['label']}: {val}")

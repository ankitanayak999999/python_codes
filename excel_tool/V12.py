...
def get_user_inputs_dynamic(fields):
    ...
    def submit():
        ...
        result = [e.get() if isinstance(e, (tk.Entry, ttk.Combobox, tk.StringVar)) else "" for e in entries]
        root.destroy()
    ...
    for i, field in enumerate(fields):
        ...
        if ftype == "dropdown":
            ...
            if options:
                var.set(options[0])  # Set default selection
    ...
    return [e.get() if isinstance(e, (tk.Entry, ttk.Combobox, tk.StringVar)) else "" for e in entries]
